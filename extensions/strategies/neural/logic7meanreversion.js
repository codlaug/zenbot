...
module.exports = function(s, weights, cb) {
  ...
  const { atr, pdi, emaRsi, trendMagicSignal2 } = s.period
  ...
  if(s.highestProfit > 0.005 && profitSlide > 0.1 + s.profitSlideTolerance) {
    s.signal = 'sell'
    s.profitSlideTolerance = 0
  }

  if(s.highestProfit > 0.01 && profitSlide > 0.1 + s.profitSlideTolerance) {
    s.signal = 'sell'
    s.profitSlideTolerance = 0
  }

  if(curProfit > 0.01) {
    s.profitSlideTolerance = 0.3
  }
  if(curProfit > 0.02) {
    s.profitSlideTolerance = 0.6
  }
  ...
  // TODO: use emaRsi
  // TODO: use supere treend
  // TODO: use trend magic
  // buyTrendCount still useful?
  ...
  if(downwardTrend) {
    ...
    if(trendMagicSignal2 === 1) {
      if(veryLongEmaChange > 0.1) {
        s.logStream.write(`emaDiff: ${emaDiff}\n`)
        s.logStream.write(`longEma: ${longEmaChange}\n`)
        s.signal = 'buy'
      }
    }
