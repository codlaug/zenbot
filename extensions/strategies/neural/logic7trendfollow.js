...
module.exports = function(s, weights, cb) {
  ...
  if(upwardTrend) {
    ...
    if(recentlyCrossed && veryLongEmaChange > 0.2 && ultraLongEmaChange > 0.2 && emaDiff < 20) {
      // s.logStream.write(`emaDiff: ${emaDiff}\n`)
      s.signal = 'buy'
    }

    if(s.highestProfit > 0.020 && curProfit > 0.010 && profitSlide > 0.3) {
      s.logStream.write('large profit slide sell\n')
      s.logStream.write(`emaDiff: ${emaDiff}\n`)
      if(!emasStillRising) {
        s.signal = 'sell'
      }
    ...
    if(anyFear && s.highestProfit > 0.008 && profitSlide > 0.15) {
      s.logStream.write('small profit slide sell\n')
      s.logStream.write(`emaDiff: ${emaDiff}\n`)
      // if(!emasStillRising) {
        s.signal = 'sell'
      // }
    ...
    if(emasConverging && predictedVeryLong < predictedUltraLong - 2) {
  ...
  if(downwardTrend) {
    ...
    // s.logStream.write(`predictedVeryLong > predictedUltraLong: ${predictedVeryLong - predictedUltraLong}\n`)  
    if(predictedVeryLong > predictedUltraLong + 2 && atr < 10) {
      s.logStream.write('predicted cross buy from downtrend\n')
      // s.logStream.write(`atr: ${atr}`)
      s.signal = 'buy'
    }

    if(emasDiverging && predictedVeryLong < predictedUltraLong - 2) {
      // s.logStream.write(`predictedVeryLong < predictedUltraLong: ${predictedVeryLong} < ${predictedUltraLong}\n`)
      console.log('predictedVeryLong', predictedVeryLong)
      console.log('predictedUltraLong', predictedUltraLong)
      s.signal = 'sell'
