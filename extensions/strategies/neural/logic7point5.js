...
module.exports = function(s, weights, cb) {
  ...
  if(s.lookback.length > 16) {
    ...
    const ultraLongEmaZ = smoothedZScore(closeArray, {threshold: 1.8, lag: length-2})
  ...
  const length = Math.min(s.lookback.length, 32)
  ...
  const predictedVeryLong = modelVeryLong.predict(length+20)[1]
  ...
  const predictedUltraLong = modelUltraLong.predict(length+20)[1]
  ...
  if(upwardTrend) {
    ...
    if(!longSellSig) {
      s.actedOnSellSig = false
      s.buyTrendCount = 0
    }

    if(lowLongPeakSig) {
      console.log('longSellSig', longSellSig, s.buyTrendCount)
      console.log('atr', atr)
      s.buyTrendCount += 1
      const volatileEnough = atr > 4.0
      const trendingEnough = pdi > 25
      if(s.buyTrendCount < 90 && emasDiverging) {
        if(trendingEnough && volatileEnough && !s.actedOnSellSig) {
          // this was bad in Feb2-Feb9
          // might have helped in Jan12-19?
          const earlyInThePeakRise = emaDiff > -50 // this could be better during Fear???
          if(earlyInThePeakRise || true) {
            ...
            // s.logStream.write(`emaDiffChange: ${emaDiffChange}\n`)
            // s.logStream.write(`emaDiffChangeChange: ${emaDiffChangeChange}\n`)
            // s.logStream.write(`emaDiff: ${emaDiff}\n`)
            // s.logStream.write(`emaDivergance: ${emaDivergance}\n`)
            s.actedOnSellSig = true
            // s.signal = 'buy'
          }
        }
      }
    ...
    if(emasCrossed) {
      ...
      s.logStream.write(`emaCross buy: ${pdi}\n`)
      // s.logStream.write(`atr: ${atr}\n`)
      // s.logStream.write(`longEmaChange: ${longEmaChange}\n`)
      // s.logStream.write(`veryLongEmaChange: ${veryLongEmaChange}\n`)
      // s.logStream.write(`ultraLongEmaChange: ${ultraLongEmaChange}\n`)
    ...
    // if(s.emaCrossTrendCount >= 0 && s.emaCrossTrendCount < 20) {
    //   s.emaCrossTrendCount += 1
    //   s.logStream.write(`emaDivergance: ${emaDivergance}\n`)
    //   s.logStream.write(`atr: ${atr}\n`)
    //   if(emaDivergance < -0.3 && atr > 3.0) {
    //     s.signal = 'buy'
    //   }
    // }
    ...
    if(atr < 8.0) {
      if(s.highestProfit > 0.006 && curProfit > 0.003 && profitSlide > 0.10) {
        // if(rsi < emaRsi) {
        //   if(emaRsi > 40 && emaRsi < 60) {
        s.logStream.write(`larger sell emaDiff: ${emaDiff}`)
        s.logStream.write(`small sell atr: ${atr}`)
        // s.signal = 'sell'
        //   }
        // }
      }
    ...
    if(s.highestProfit > 0.020 && curProfit > 0.023 && profitSlide > 0.15) {
      if(emaDiff > -20) {
        ...
        //   if(emaRsi > 40 && emaRsi < 60) {
        s.logStream.write(`larger sell emaDiff: ${emaDiff}`)
        s.logStream.write(`larger sell atr: ${atr}`)
        // s.signal = 'sell'
        //   }
        // }
      }
    ...
    if(emaDiffPeakSig) {
      s.highestEmaDiffChange = Math.max(s.highestEmaDiffChange, s.period.emaDiffChangeEma)
      // console.log('highestEmaDiffChange', s.highestEmaDiffChange)
      // s.logStream.write(`highestEmaDiffChange: ${s.highestEmaDiffChange}\n`)
      if(s.highestEmaDiffChange < -0.4) {
        // console.log('emaDiffChangeChange', emaDiffChangeChange)
        // s.logStream.write(`emaDifChangeChange: ${emaDiffChangeChange}\n`)
        if(emaDiffChangeChange > -0.01) {
    ...
    if(someFear) {
      ...
      if(s.highestProfit > 0.013 && curProfit > 0.008 && profitSlide > 0.3) {
        // s.signal = 'sell'
      }
    }

    if(emasConverging && predictedVeryLong < predictedUltraLong - 29) {
      // s.logStream.write(`predictedVeryLong < predictedUltraLong: ${predictedVeryLong} < ${predictedUltraLong}\n`)
      console.log('predictedVeryLong', predictedVeryLong)
      console.log('predictedUltraLong', predictedUltraLong)
      // s.signal = 'sell'
    }
  ...
  if(downwardTrend) {
    ...
    s.highestEmaDiff = Math.max(s.highestEmaDiff, emaDiff)
    const sufficientEmaDiff = s.highestEmaDiff > 30
    // s.logStream.write(`s.highestEmaDiff ${s.highestEmaDiff}\n`)
    ...
    s.logStream.write(`predictedVeryLong > predictedUltraLong: ${predictedVeryLong - predictedUltraLong}\n`)  
    if(predictedVeryLong > predictedUltraLong + 2) {
      s.signal = 'buy'
    }



    if(emaDiffPeakSig && longBuySig && longEmaChange > -1.8) {
      // s.signal = 'buy'
      if(assets < 0.05) {
        s.patience = 0
      }
      // s.logStream.write(`longEmaChange: ${longEmaChange}\n`)
      // s.logStream.write(`veryLongEmaChange: ${veryLongEmaChange}\n`)
      if(someFear || veryLongEmaChange < -0.4) {
    ...
    if(!greed && emaDiffTroughSig && tSell > 0.46 && veryLongEmaChange < 0.1 && emaDiff < 10) {
      // s.logStream.write(`emaDiffTrough sell: ${veryLongEmaChange}\n`)
      s.signal = 'sell'
    }

    if(tSell > 0.46 && veryLongEmaChange < 0.1 && emaDiff < 10) {
      // s.logStream.write(`tSell 0.46 sell: ${veryLongEmaChange}\n`)
      s.signal = 'sell'
    }

    if(someFear && emasDiverging) {
      // big profit slide - just sell
      if(s.highestProfit > 0.010 && profitSlide > 0.3) {
        // s.logStream.write(`profitSlide sell: ${veryLongEmaChange}\n`)
        s.signal = 'sell'
      }
    }


    console.log(s.patience)
    if(s.patience > 180 && s.highestProfit < 0.01 && profitSlide > 1.5 && tSell > 0.46) {
      // s.logStream.write(`atr: ${atr}\n`)
      // s.signal = 'sell'
    }
